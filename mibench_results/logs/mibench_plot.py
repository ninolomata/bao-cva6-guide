...
rx_dict = {
    ...
    'dataset': re.compile(r'(?P<dataset>^\w+\-(small|large))'),
...
#ap.add_argument("-p", "--rootdir", required=True,
#   help="Pointer to the benchmarks root dir")
#ap.add_argument("-s", "--save_to_csv",
...
def plot_bw_bench(data):
    ...
    norm_data.to_csv(rootdir + "/" + "norm_data.csv")
    # get xticks labels
    ...
    for i,scene in enumerate(scenarios) :
        ...
        axs.set_title("Mibench Automotive Subset Performance Evaluation",fontsize=25)
        #axs.set_title(size)
    ...
    plt.subplots_adjust(top=0.935,
                        ...
                        hspace=0.34,
                        wspace=0.22)
    plt.legend(ncol=4,fontsize=25, loc='upper left')
    pdf.savefig(fig)
...
def plot_event_bench(data,dataset):
    ...
    for i,scene in enumerate(scenarios) :
        ...
        axs.legend(ncol=2,fontsize=35, loc='upper left')
